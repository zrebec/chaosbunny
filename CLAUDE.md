...
3. **Dev linking — RESOLVED.** zx-kit's perf/clash modules ship in `0.29.0` (`cache`), `0.30.0` (`attrscreen`) and `0.31.0` (`monoscreen`); this repo tracks `"zx-kit": "^0.31.0"`. (When bumping zx-kit during dev, run `npm run dev -- --force` so Vite re-bundles.)
4. **Colour / clash modes — DONE.** `C` toggles a monochrome playfield: the whole play area is drawn into a zx-kit `MonoScreen` via a small `Painter` abstraction (`src/world/clash.ts`: `ctxPainter` = full colour, `monoPainter` = mono), so **every** entity (tiles, rabbit, spiders + thread via `fillMono`, bats, carrot shots, torches) becomes one ink/paper — no clash; white-spider-on-cyan and invisible shots both fixed. The default (clash-off) view is byte-identical full colour. We chose mono over zx-kit's per-cell `attrscreen` clash because the 2×3-cell rabbit looks bad clashed; `attrscreen` stays in the kit for games that want true clash. GPU 31–35% in clash. **Pending (later):** smaller playfield + reserved colour HUD bar (`monoScreen` already supports its own size + offset); carrot redesign (~10×8, yellow body + B_GREEN leaves); optional sparse mono bricks.
